...
export function TextSearchSimulatorComponent() {
  const [text, setText] = useState('')
  const [algorithms, setAlgorithms] = useState<AlgorithmItem[]>([
    { id: 1, algorithm: 'naive', pattern: '', isRunning: false, highlights: [] }
  ])

  const addAlgorithm = () => {
    const newId = Math.max(...algorithms.map(a => a.id), 0) + 1
    setAlgorithms([...algorithms, { id: newId, algorithm: 'naive', pattern: '', isRunning: false, highlights: [] }])
  }
  ...
  const addHighlight = (id: number) => {
    ...
    if (algo) {
      const color = highlightColors[algo.highlights.length % highlightColors.length]
      const newHighlight = { start: 0, end: Math.min(10, text.length), color }
      updateAlgorithm(id, { highlights: [...algo.highlights, newHighlight] })
    }
  }

  const updateHighlight = (algoId: number, index: number, start: number, end: number) => {
    setAlgorithms(algorithms.map(algo => 
      algo.id === algoId 
        ? { ...algo, highlights: algo.highlights.map((h, i) => 
    ...
    <div className="p-4 max-w-4xl mx-auto">
      ...
      <div className="mb-4 p-4 border rounded-lg bg-gray-50 min-h-[200px] relative">
        {text.split('').map((char, index) => {
          ...
          const style = highlights.length > 0 ? { backgroundColor: highlights[0].color } : {}
        ...
        {algorithms.map((algo) => (
          <div key={algo.id} className="flex items-center space-x-2">
            <Select
              ...
              onValueChange={(value: Algorithm) => updateAlgorithm(algo.id, { algorithm: value })}
            >
            ...
            <Input
              ...
              value={algo.pattern}
              onChange={(e) => updateAlgorithm(algo.id, { pattern: e.target.value })}
              className="w-[180px]"
            />
            <Button variant="outline" size="icon" onClick={() => toggleAlgorithm(algo.id)}>
              {algo.isRunning ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
            </Button>
            <Button variant="outline" size="icon" onClick={() => addHighlight(algo.id)}>
              <Plus className="h-4 w-4" />
            </Button>
            <Button variant="outline" size="icon" onClick={() => removeAlgorithm(algo.id)} disabled={algorithms.length === 1}>
            ...
            {algo.highlights.map((highlight, index) => (
              <div key={index} className="flex items-center space-x-2">
                ...
                <Input
                  ...
                  onChange={(e) => updateHighlight(algo.id, index, parseInt(e.target.value), highlight.end)}
                  className="w-16"
                />
                ...
                <Input
                  type="number"
                  value={highlight.end}
                  onChange={(e) => updateHighlight(algo.id, index, highlight.start, parseInt(e.target.value))}
                  className="w-16"
                />
              </div>
            ))}
